...
// import { handleFirebaseError, createFirebaseErrorBoundary } from './firebase-error-handler'
...
class BannerSyncService {
  ...
  private validateBannerData(banner: BannerData): any {
    ...
    // Se a imagem for uma URL de dados (base64), fazer upload para o Firebase Storage
    if (banner.currentImage && banner.currentImage.startsWith('data:image')) {
      try {
        // Upload da imagem para o Firebase Storage
        const downloadURL = firebaseStorageService.uploadBanner(banner.currentImage, banner.id);
        bannerData.imageUrl = downloadURL;
      } catch (error) {
        console.error('Erro ao fazer upload da imagem do banner:', error);
        // Manter a imagem original se o upload falhar
        bannerData.imageUrl = banner.currentImage;
      }
    }
  ...
  //     // handleFirebaseError(error, `Banner Sync ${banner.id}`)
  //     
  ...
  //     const hasFirebaseBanners = false
  //     
