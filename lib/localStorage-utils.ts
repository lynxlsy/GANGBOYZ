...
export class LocalStorageManager {
  ...
  static checkStorageQuota(): { used: number; total: number; available: number } {
    try {
      ...
      // Estimate total quota (typically 5-10MB in modern browsers)
  ...
  // Clean old banner data to free up space
  static cleanupOldBannerData() {
    try {
      const storageKeys = [
      ...
      storageKeys.forEach(storageKey => {
        ...
        if (savedData) {
          ...
          if (Array.isArray(data)) {
            ...
            if (dataSize > 4.5 * 1024 * 1024) { // 4.5MB limit
              // Keep only the 5 most recent items
              const recentData = data.slice(-5);
              localStorage.setItem(storageKey, JSON.stringify(recentData));
              console.log(`✅ Old data cleaned for ${storageKey} due to size limit`);
              cleaned = true;
            } else if (data.length > 10) {
              // Keep only the 10 most recent items
              const recentData = data.slice(-10);
              localStorage.setItem(storageKey, JSON.stringify(recentData));
              console.log(`✅ Old data cleaned for ${storageKey}`);
              cleaned = true;
            }
          }
        }
      ...
      console.error('Error cleaning old banner data:', error);
      return false;
    }
  }
  
  // Optimize banner data to reduce storage size
  static optimizeBannerData(banner: any): any {
    // Remove unnecessary properties and optimize data
    const optimized: any = {
      id: banner.id,
      name: banner.name,
    ...
    if (banner.cropMetadata && Object.keys(banner.cropMetadata).length > 0) {
      optimized.cropMetadata = banner.cropMetadata;
    }
    
    if (banner.overlaySettings && Object.keys(banner.overlaySettings).length > 0) {
      optimized.overlaySettings = banner.overlaySettings;
    }
  ...
  static safeSetItem(key: string, value: string): boolean {
    try {
      ...
      if (quota.available < 100 * 1024) { // Less than 100KB available
        console.warn('Low storage space, attempting cleanup...');
        this.cleanupOldBannerData();
      }
