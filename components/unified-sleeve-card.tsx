...
interface UnifiedSleeveCardProps {
  product: Product
...
export function UnifiedSleeveCard({ 
  product, 
  onAddToCart, 
  className = "", 
  isEditable = false, 
  onEdit 
}: UnifiedSleeveCardProps) {
  ...
  // Determine sleeve type based on product name or category
  const getSleeveType = () => {
    const name = product.name.toLowerCase()
    const subcategory = product.categories.find(cat => cat)?.toLowerCase() || ""
    
    if (name.includes("regata") || subcategory.includes("regata")) {
      return "Regata"
    } else if (name.includes("manga curta") || subcategory.includes("manga curta")) {
      return "Manga Curta"
    } else if (name.includes("manga longa") || subcategory.includes("manga longa")) {
      return "Manga Longa"
    }
    
    // Default to subcategory if available
    return product.categories[0] || "Produto"
  }

  const sleeveType = getSleeveType()
  ...
  return (
    ...
    >
      ...
      <div className="w-[240px] h-[280px] bg-gray-800 flex items-center justify-center overflow-hidden relative group">
        ...
        {/* Tipo de manga - identificação visual */}
        <div className="absolute top-2 left-2 px-2 py-1 rounded-md text-xs font-bold text-white bg-blue-500">
          {sleeveType}
        </div>
